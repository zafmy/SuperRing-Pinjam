...
export const api = {
  ...
  pushConfig: (code: string, token: string) => request<{ enabled: boolean; publicKey: string | null }>(`/sessions/${encodeURIComponent(code)}/push/config`, { headers: { Authorization: `Bearer ${token}` } }),
  pushAction: (code: string, token: string, action: 'subscribe' | 'unsubscribe' | 'status' | 'test', input: unknown) => request<{ subscribed?: boolean; accepted?: boolean; lastError?: string | null; lastSentAt?: string | null }>(`/sessions/${encodeURIComponent(code)}/push/${action}`, { method: 'POST', headers: { Authorization: `Bearer ${token}` }, body: JSON.stringify(input) }),
