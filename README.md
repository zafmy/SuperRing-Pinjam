...
**Shared starter, not the completed agent.** The server can create a session, accept participants and return an authenticated shared snapshot. Data survives a server restart on the same disk. The browser currently contains a connection check for frontend handoff.

QR screens, photo upload, task actions, AI planning and visual verification are not built yet. See [STATUS](docs/STATUS.md) for the current checkpoint.
